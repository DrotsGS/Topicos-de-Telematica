...
def nodo():
    ...
    yield nn_grpc.NameNodeServiceStub(canal), servicio
...
def test_login_ok(nodo):
    stub, _ = nodo
    r = stub.Login(nn.LoginRequest(user="drots", password="dfsha"))
...
def test_login_password_mala(nodo):
    ...
    with pytest.raises(grpc.RpcError) as e:
        stub.Login(nn.LoginRequest(user="drots", password="nope"))
...
def test_login_usuario_inexistente(nodo):
    stub, _ = nodo
    with pytest.raises(grpc.RpcError) as e:
        stub.Login(nn.LoginRequest(user="nadie", password="x"))
    assert codigo(e) == grpc.StatusCode.UNAUTHENTICATED
...
@pytest.mark.parametrize("llamada", [
    lambda s, t: s.Mkdir(nn.PathRequest(path="/x", token=t)),
    lambda s, t: s.Ls(nn.PathRequest(path="/", token=t)),
    lambda s, t: s.Rmdir(nn.PathRequest(path="/x", token=t)),
    lambda s, t: s.Rm(nn.PathRequest(path="/x", token=t)),
    lambda s, t: s.Stat(nn.PathRequest(path="/", token=t)),
])
...
def test_mkdir_duplicado_es_already_exists(sesion):
    stub, _, t = sesion
    stub.Mkdir(nn.PathRequest(path="/docs", token=t))
    with pytest.raises(grpc.RpcError) as e:
        stub.Mkdir(nn.PathRequest(path="/docs", token=t))
...
def test_mkdir_sin_padre_es_not_found(sesion):
    stub, _, t = sesion
    with pytest.raises(grpc.RpcError) as e:
        stub.Mkdir(nn.PathRequest(path="/a/b/c", token=t))
...
def test_recorrido_completo(sesion):
    ...
    stub, _, t = sesion
    P = lambda p: nn.PathRequest(path=p, token=t)   # noqa: E731

    stub.Mkdir(P("/docs"))
    stub.Mkdir(P("/docs/2026"))

    info = stub.Stat(P("/docs"))
    assert info.is_dir and info.state == comun.COMMITTED
...
def test_rmdir_no_existe_es_not_found(sesion):
    stub, _, t = sesion
    with pytest.raises(grpc.RpcError) as e:
        stub.Rmdir(nn.PathRequest(path="/nada", token=t))
    assert codigo(e) == grpc.StatusCode.NOT_FOUND
...
def test_rmdir_sobre_archivo_es_invalid_argument(sesion):
    ...
    with pytest.raises(grpc.RpcError) as e:
        stub.Rmdir(nn.PathRequest(path="/f", token=t))
...
def test_rmdir_raiz_es_failed_precondition(sesion):
    stub, _, t = sesion
    with pytest.raises(grpc.RpcError) as e:
        stub.Rmdir(nn.PathRequest(path="/", token=t))
    assert codigo(e) == grpc.StatusCode.FAILED_PRECONDITION


def test_rm_sobre_directorio_es_invalid_argument(sesion):
    stub, _, t = sesion
    stub.Mkdir(nn.PathRequest(path="/d", token=t))
    with pytest.raises(grpc.RpcError) as e:
        stub.Rm(nn.PathRequest(path="/d", token=t))
    assert codigo(e) == grpc.StatusCode.INVALID_ARGUMENT
...
def test_rm_agenda_los_bloques_huerfanos(sesion):
    ...
    assert stub.Rm(nn.PathRequest(path="/f", token=t)).ok
...
def test_rm_de_un_archivo_en_construccion(sesion):
    ...
    r = stub.Rm(nn.PathRequest(path="/f", token=t))
...
def test_stat_no_existe_es_not_found(sesion):
    stub, _, t = sesion
    with pytest.raises(grpc.RpcError) as e:
        stub.Stat(nn.PathRequest(path="/nada", token=t))
    assert codigo(e) == grpc.StatusCode.NOT_FOUND
...
def test_stat_ve_un_archivo_en_construccion(sesion):
    ...
    info = stub.Stat(nn.PathRequest(path="/f", token=t))
    assert info.state == comun.UNDER_CONSTRUCTION and info.size == 42
    assert list(stub.Ls(nn.PathRequest(path="/", token=t)).entries) == []
